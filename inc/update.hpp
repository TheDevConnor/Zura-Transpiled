...
void installer() {
    ...
    if (os_name == "windows") {
        std::string url = "https://github.com/TheDevConnor/Zura-Transpiled/releases/download/";
        ...
        try {
            ...
            std::cout << "Please enter your password to install Zura. For the first time" << std::endl;
            system(command.c_str());
        } catch (const std::exception& e) {
            std::cout << "Error running command: " << e.what() << std::endl;
            return;
