...
public:
    static void error(Lexer::Token line, std::string msg, Lexer& lexer) {
        ...
        std::cout << termcolor::yellow << "Error" << termcolor::reset 
                  ...
                  << ", column: " << termcolor::blue << line.column - 1 << termcolor::reset << "] "  
        ...
        while (*lineEnd != '\n' && *lineEnd != '\0') lineEnd++;
        std::cout << std::endl << std::string(lineStart, lineEnd - lineStart) << std::endl;

        // The arrow pointing to the error
        std::string length(lineStart, lineEnd - lineStart);
        std::cout << termcolor::green << std::string(line.column - 2, '~') << termcolor::red << "^" << termcolor::reset << std::endl;
