...
Node::Stmt *Parser::exprStmt(PStruct *psr) {
  ...
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a expr stmt!");
...
Node::Stmt *Parser::blockStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::LEFT_BRACE, "Expected a L_BRACE to start a block stmt");
  ...
  while (psr->current().kind != TokenKind::RIGHT_BRACE) {
    ...
    if (pastKind == NodeKind::ND_IF_STMT || pastKind == NodeKind::ND_WHILE_STMT || pastKind == NodeKind::ND_FOR_STMT || pastKind == NodeKind::ND_BLOCK_STMT || pastKind == NodeKind::ND_FN_STMT) {
...
Node::Stmt *Parser::varStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::VAR, "Expected a VAR or CONST keyword to start a var stmt");

  name = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER after a VAR or CONST keyword").value;

  psr->expect(TokenKind::COLON, "Expected a COLON after the variable name in a var stmt to define the type of the variable");
  Node::Type *varType = parseType(psr);

  if (psr->current().kind == TokenKind::SEMICOLON) {
    psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a var stmt");
    return new VarStmt(line, column, isConst, name, varType, nullptr, codegen::getFileID(psr->current_file));
  }

  psr->expect(TokenKind::EQUAL, "Expected a EQUAL after the type of the variable in a var stmt");
...
Node::Stmt *Parser::printStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::PRINT, "Expected a PRINT keyword to start an output stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start an output stmt");

  Node::Expr *fileDescriptor = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the file descriptor in an output stmt");
  ...
  while (psr->current().kind != TokenKind::RIGHT_PAREN) {
    ...
    if (psr->current().kind == RIGHT_PAREN) break;
    psr->expect(TokenKind::COMMA, "Expected a COMMA after an arguement in an output stmt");
  }

  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end an output stmt");
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of an output stmt");
...
Node::Stmt *Parser::printlnStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::PRINTLN, "Expected a PRINTLN keyword to start an output stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start an output stmt");
...
Node::Stmt *Parser::constStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::_CONST, "Expected a CONST keyword to start a const stmt");
  name = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER after a CONST keyword").value;

  psr->expect(TokenKind::WALRUS, "Expected a WALRUS after the variable name in a const stmt");
...
Node::Stmt *Parser::funStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::FUN, "Expected a FUN keyword to start a function stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start a function stmt");

  std::vector<std::pair<std::string, Node::Type *>> params;
  while (psr->current().kind != TokenKind::RIGHT_PAREN) {
    std::string paramName = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a parameter name in a function stmt").value;
    psr->expect(TokenKind::COLON, "Expected a COLON after the parameter name in a function stmt");
    ...
    psr->expect(TokenKind::COMMA, "Expected a COMMA after a parameter in a function stmt");
  }
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end the parameters in a function stmt");
...
Node::Stmt *Parser::returnStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::RETURN, "Expected a RETURN keyword to start a return stmt");
  if (psr->peek().kind == TokenKind::SEMICOLON) {
    psr->advance();
    return new ReturnStmt(line, column, nullptr, codegen::getFileID(psr->current_file));
  }
  Node::Expr *expr = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a return stmt");
...
Node::Stmt *Parser::ifStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start the condition in an if stmt");
  Node::Expr *condition = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end the condition in an if stmt");

  Node::Stmt *thenStmt = parseStmt(psr, name);
  Node::Stmt *elseStmt = nullptr;

  if (psr->current().kind == TokenKind::ELSE) {
    psr->expect(TokenKind::ELSE, "Expected an ELSE keyword to start the else stmt");
...
Node::Stmt *Parser::structStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::STRUCT, "Expected a STRUCT keyword to start a struct stmt");
  psr->expect(TokenKind::LEFT_BRACE, "Expected a L_BRACE to start a struct stmt");
  ...
  while (psr->current().kind != TokenKind::RIGHT_BRACE) {
    ...
    switch (tokenKind) {
      case TokenKind::IDENTIFIER: {
        std::string fieldName = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a field name in a struct stmt").value;
        ...
        psr->expect(TokenKind::COLON, "Expected a COLON after the field name in a struct stmt");
        Node::Type *fieldType = parseType(psr);
        fields.push_back({fieldName, fieldType});
        if (psr->peek().kind == TokenKind::RIGHT_BRACE) break;  // who cares about the semicolon/comma?
        if (warnForSemi) {
          psr->expect(TokenKind::COMMA, "Semicolons are non-standard for struct field lists; use commas instead");
        } else {
          if (psr->current().kind == TokenKind::COMMA) {
            psr->advance();
          }
          psr->expect(TokenKind::SEMICOLON, "Expected a comma or a semicolon for struct field list");
        }
        break;
      }
      case TokenKind::SEMICOLON: {
        psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON after a field in a struct stmt");
        break;
      }
  ...
  psr->expect(TokenKind::RIGHT_BRACE, "Expected a R_BRACE to end a struct stmt");
...
Node::Stmt *Parser::loopStmt(PStruct *psr, std::string name) {
  ...
  while (psr->current().kind != TokenKind::RIGHT_PAREN) {
    ...
    if (psr->peek(1).kind == TokenKind::EQUAL) {
      ...
      psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON after the for loop initializer in a loop stmt");
      condition = parseExpr(psr, BindingPower::defaultValue);
    } else {
      whileLoop = parseExpr(psr, BindingPower::defaultValue);
    }

    psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end the condition in a loop stmt");
    if (psr->current().kind == TokenKind::COLON) {
      isOptional = true;
      psr->expect(TokenKind::COLON, "Expected a COLON to start the body of a loop stmt");
      psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start the condition in a loop stmt");
...
Node::Stmt *Parser::matchStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::MATCH, "Expected a MATCH keyword to start a match stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start a match stmt catch");
  Node::Expr *cond = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end the condition in a match stmt");
  psr->expect(TokenKind::LEFT_BRACE, "Expected a L_BRACE to start a match stmt body");
  ...
  while (psr->current().kind != TokenKind::RIGHT_BRACE) {
    if (psr->current().kind == TokenKind::DEFAULT) {
      psr->advance();
      psr->expect(TokenKind::RIGHT_ARROW, "Expected a RIGHT_ARROW after the DEFAULT keyword in a match stmt");
      ...
      if (psr->current().kind == TokenKind::SEMICOLON) psr->advance();
      continue;
    }
    if (psr->current().kind == TokenKind::CASE) {
      psr->advance();
      Node::Expr *caseExpr = parseExpr(psr, BindingPower::defaultValue);
      psr->expect(TokenKind::RIGHT_ARROW, "Expected a RIGHT_ARROW after the case expression in a match stmt");
      ...
      std::string msg = "Unexpected token in match statement: " + psr->current().value;
      ErrorClass::error(line, column, msg, "", "Parser Error",
                        psr->current_file.c_str(), lexer, psr->tks, true,
                        false, false, false, false, false);
...
Node::Stmt *Parser::enumStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::ENUM, "Expected an ENUM keyword to start an enum stmt");
  psr->expect(TokenKind::LEFT_BRACE, "Expected a L_BRACE to start an enum stmt");

  std::vector<std::string> fields;
  bool warnForSemi = true;
  while (psr->current().kind != TokenKind::RIGHT_BRACE) {
    Lexer::Token ident = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a field name in an enum stmt");
    ...
    if (psr->current().kind == TokenKind::RIGHT_BRACE) break;
    if (psr->current().kind == TokenKind::SEMICOLON) {
      ...
      psr->expect(TokenKind::COMMA, "Expected a COMMA after a field name in an enum stmt");
  }
  psr->expect(TokenKind::RIGHT_BRACE, "Expected a R_BRACE to end an enum stmt");
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of an enum stmt");
...
Node::Stmt *Parser::templateStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::TEMPLATE, "Expected a TEMPLATE keyword to start a template stmt");
  psr->expect(TokenKind::LESS, "Expected a '<' to start a template stmt");

  std::vector<std::string> typeParams;
  while (psr->current().kind != TokenKind::GREATER) {
    psr->expect(TokenKind::TYPEALIAS, "Expected a TYPEALIAS keyword as a type parameter in a template stmt");
    typeParams.push_back(psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a type parameter in a template stmt").value);
    if (psr->current().kind == TokenKind::GREATER) break;
    psr->expect(TokenKind::COMMA, "Expected a COMMA after a type parameter in a template stmt");
  }

  psr->expect(TokenKind::GREATER, "Expected a '>' to end a template stmt");
...
Node::Stmt *Parser::importStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::IMPORT, "Expected an IMPORT keyword to start an import stmt");
  std::string current_file = node.current_file;
  std::string path = psr->expect(TokenKind::STRING, "Expected a STRING as a path in an import stmt").value;
  ...
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of an import stmt");
...
Node::Stmt *Parser::linkStmt(PStruct *psr, std::string name) {
  ...
  std::string path = psr->expect(TokenKind::STRING, "Expected a STRING as a path in a link stmt").value;
  path.erase(path.begin());    // Erase initial "
  path.erase(path.end() - 1);  // Erase final "
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a link stmt");
...
Node::Stmt *Parser::externStmt(PStruct *psr, std::string name) {
  ...
  // @extern <"", "", "">; or @extern "C";
  psr->expect(TokenKind::EXTERN, "Expected an EXTERN keyword to start an extern stmt");
  if (psr->current().kind == TokenKind::LESS) {
    psr->expect(TokenKind::LESS, "Expected a LESS to start an extern stmt");
    while (psr->current().kind != TokenKind::GREATER) {
      std::string path = psr->expect(TokenKind::STRING, "Expected a STRING as a path in an extern stmt").value;
      path.erase(path.begin());    // Erase initial "
      path.erase(path.end() - 1);  // Erase final "
      externs.push_back(path);
      if (psr->current().kind == TokenKind::GREATER) break;
      psr->expect(TokenKind::COMMA, "Expected a COMMA after a path in an extern stmt");
    }
    psr->expect(TokenKind::GREATER, "Expected a GREATER to end an extern stmt");
    psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of an extern stmt");
...
Node::Stmt *Parser::breakStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::BREAK, "Expected a BREAK keyword to start a break stmt");
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a break stmt");
...
Node::Stmt *Parser::continueStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::CONTINUE, "Expected a CONTINUE keyword to start a continue stmt");
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of a continue stmt");
...
Node::Stmt *Parser::inputStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::INPUT, "Expected an INPUT keyword to start an input stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start an input stmt");

  Node::Expr *fileDescriptor = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the file descriptor in an input stmt");

  Node::Expr *bufferOut = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the variable in an input stmt");

  Node::Expr *maxBytes = parseExpr(psr, BindingPower::defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_PAREN to end an input stmt");
  psr->expect(TokenKind::SEMICOLON, "Expected a SEMICOLON at the end of an input stmt");
  return new InputStmt(line, column, fileDescriptor, bufferOut, maxBytes, codegen::getFileID(psr->current_file));
}
...
Node::Stmt *Parser::closeStmt(PStruct *psr, std::string name) {
  ...
  psr->expect(TokenKind::CLOSE, "Expected a CLOSE keyword to start a close stmt");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_PAREN to start a close stmt");
