...
std::vector<AstNode*> Parser::lookupMain() {
    ...
    for (AstNode* stmt : statements) {
        if (stmt->type == AstNodeType::FUNCTION_DECLARATION) {
            AstNode::FunctionDeclaration* function = (AstNode::FunctionDeclaration*)stmt->data;
            if (function->name.start[0] == 'm' && function->name.start[1] == 'a' && function->name.start[2] == 'i' && function->name.start[3] == 'n') {
                main = stmt;
                break;
            }
        }
    ...
    if (!main) ParserError::error(previousToken, "No main function found", lexer);
