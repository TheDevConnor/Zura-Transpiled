...
Node::Expr *Parser::primary(PStruct *psr) {
  ...
  switch (psr->current().kind) {
    case TokenKind::INT: {
      return new IntExpr(line, column, std::stoll(psr->advance().value), codegen::getFileID(psr->current_file));
    }
    case TokenKind::FLOAT: {
      return new FloatExpr(line, column, psr->advance().value, codegen::getFileID(psr->current_file));
    }
    case TokenKind::IDENTIFIER: {
      return new IdentExpr(line, column, psr->advance().value, nullptr, codegen::getFileID(psr->current_file));
    }
    case TokenKind::STRING: {
      return new StringExpr(line, column, psr->advance().value, codegen::getFileID(psr->current_file));
    }
    case TokenKind::CHAR: {
...
Node::Expr *Parser::group(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected L_Paran before a grouping expr!");
  Node::Expr *expr = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected R_Paran after a grouping expr!");
...
Node::Expr *Parser::freeExpr(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected L_Paran for a free memory expression!");

  Node::Expr *whatToFree = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the expression to free!");
  Node::Expr *bytesToFree = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected R_Paran to end a free memory expression!");
...
Node::Expr *Parser::castExpr(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_Paran to be able to cast an expression!");
...
Node::Expr *Parser::sizeofExpr(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_Paran to start a sizeof expr!");
  Node::Expr *expr = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_Paran to end a sizeof expr!");
...
Node::Expr *Parser::memcpyExpr(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_Paran to start a memcpy expr!");
  Node::Expr *dest = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the destination in a memcpy expr!");
  Node::Expr *src = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::COMMA, "Expected a COMMA after the source in a memcpy expr!");

  Node::Expr *size = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_Paran to end a memcpy expr!");
...
Node::Expr *Parser::array(PStruct *psr) {
  ...
  psr->expect(TokenKind::LEFT_BRACKET, "Expected a L_Bracket to start an array expr!");
  ...
  while (psr->current().kind != TokenKind::RIGHT_BRACKET) {
    ...
    if (psr->current().kind == TokenKind::RIGHT_BRACKET) break;
    psr->expect(TokenKind::COMMA, "Expected a COMMA after an element in an array expr!");
  }
  psr->expect(TokenKind::RIGHT_BRACKET, "Expected a R_Bracket to end an array expr!");
...
Node::Expr *Parser::index(PStruct *psr, Node::Expr *left, BindingPower bp) {
  ...
  psr->expect(TokenKind::LEFT_BRACKET, "Expected a L_Bracket to start an index expr!");

  Node::Expr *index = nullptr;
  switch (psr->current().kind) {
      // This is a pop operation
    case TokenKind::LEFT_ARROW: {
      psr->expect(TokenKind::LEFT_ARROW, "Expected a L_Arrow to pop an index from an array!");
      ...
      psr->expect(TokenKind::RIGHT_BRACKET, "Expected a R_Bracket to end an index expr!");
      return new PopExpr(line, column, left, index, codegen::getFileID(psr->current_file));
    }
    case TokenKind::RIGHT_ARROW: {
      psr->expect(TokenKind::RIGHT_ARROW, "Expected a R_Arrow to push an index to an array!");

      index = parseExpr(psr, defaultValue);

      if (psr->current().kind == TokenKind::AT) {
        psr->expect(TokenKind::AT, "Expected an AT to specify an index for a push operation!");
...
Node::Expr *Parser::parse_call(PStruct *psr, Node::Expr *left, BindingPower bp) {
  ...
  psr->expect(TokenKind::LEFT_PAREN, "Expected a L_Paran to start a call expr!");
  ...
  while (psr->current().kind != TokenKind::RIGHT_PAREN) {
    ...
    psr->expect(TokenKind::COMMA, "Expected a COMMA after an argument in a call expr!");
  }
...
Node::Expr *Parser::_ternary(PStruct *psr, Node::Expr *left, BindingPower bp) {
  ...
  psr->expect(TokenKind::QUESTION, "Expected a '?' to define the true value of an ternary expr!");
  Node::Expr *true_expr = parseExpr(psr, defaultValue);
  psr->expect(TokenKind::COLON, "Expected a ':' to define the false value of a ternary Expr!");
...
Node::Expr *Parser::externalCall(PStruct *psr) {
  ...
  std::string funcName = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a function name in a call stmt").value;

  psr->expect(TokenKind::GREATER, "Expected a GREATER to end call function name");
  psr->expect(TokenKind::LEFT_PAREN, "Expected a LEFT_PAREN to start call function arguments");

  std::vector<Node::Expr *> args;
  while (psr->current().kind != TokenKind::RIGHT_PAREN) {
    args.push_back(parseExpr(psr, BindingPower::defaultValue));
    if (psr->current().kind == TokenKind::RIGHT_PAREN) break;
    psr->expect(TokenKind::COMMA, "Expected a COMMA after an arguement in a call stmt");
  }

  psr->expect(TokenKind::RIGHT_PAREN, "Expected a RIGHT_PAREN to end call function arguments");
...
Node::Expr *Parser::structExpr(PStruct *psr) {
  int line = psr->tks[psr->pos].line;
  int column = psr->tks[psr->pos].column;
  psr->expect(TokenKind::LEFT_BRACE, "Expected a L_Brace to start a struct expr!");

  std::unordered_map<std::string, Node::Expr *> elements;
  while (psr->current().kind != TokenKind::RIGHT_BRACE) {
    std::string key = psr->expect(TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a key in a struct expr!").value;
    psr->expect(TokenKind::COLON, "Expected a COLON after a key in a struct expr!");
    Node::Expr *value = parseExpr(psr, defaultValue);
    elements[key] = value;

    if (psr->current().kind == TokenKind::RIGHT_BRACE) break;
    psr->expect(TokenKind::COMMA, "Expected a COMMA after a key-value pair in a struct expr!");
  }

  psr->expect(TokenKind::RIGHT_BRACE, "Expected a R_Brace to end a struct expr!");
...
Node::Expr *Parser::openExpr(PStruct *psr) {
  ...
  psr->expect(TokenKind::OPEN, "Expected an OPEN keyword to start an open expr!");
  ...
  psr->expect(TokenKind::RIGHT_PAREN, "Expected a R_Paren to end an open expr!");
