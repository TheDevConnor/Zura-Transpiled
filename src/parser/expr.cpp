...
Node::Expr *Parser::castExpr(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::LEFT_PAREN, "Expected a L_Paran to be able to cast an expression!");
  castee = parseExpr(psr, defaultValue);
  psr->expect(psr, TokenKind::RIGHT_PAREN, "Expected a R_Paran to end a cast expr!");
...
Node::Expr *Parser::sizeofExpr(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::LEFT_PAREN, "Expected a L_Paran to start a sizeof expr!");
  Node::Expr *expr = parseExpr(psr, defaultValue);
  psr->expect(psr, TokenKind::RIGHT_PAREN, "Expected a R_Paran to end a sizeof expr!");
...
Node::Expr *Parser::memcpyExpr(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::LEFT_PAREN, "Expected a L_Paran to start a memcpy expr!");
  Node::Expr *dest = parseExpr(psr, defaultValue);
  psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after the destination in a memcpy expr!");
  Node::Expr *src = parseExpr(psr, defaultValue);
  psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after the source in a memcpy expr!");

  Node::Expr *size = parseExpr(psr, defaultValue);
  psr->expect(psr, TokenKind::RIGHT_PAREN,  "Expected a R_Paran to end a memcpy expr!");
...
Node::Expr *Parser::array(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::LEFT_BRACKET, "Expected a L_Bracket to start an array expr!");
  ...
  while (psr->current(psr).kind != TokenKind::RIGHT_BRACKET) {
    ...
    if (psr->current(psr).kind == TokenKind::RIGHT_BRACKET) break;
    psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after an element in an array expr!");
  }
  psr->expect(psr, TokenKind::RIGHT_BRACKET, "Expected a R_Bracket to end an array expr!");
...
Node::Expr *Parser::index(PStruct *psr, Node::Expr *left, BindingPower bp) {
  ...
  psr->expect(psr, TokenKind::LEFT_BRACKET, "Expected a L_Bracket to start an index expr!");

  Node::Expr *index = nullptr;
  switch (psr->current(psr).kind) {
    // This is a pop operation
  case TokenKind::LEFT_ARROW: {
    psr->expect(psr, TokenKind::LEFT_ARROW, "Expected a L_Arrow to pop an index from an array!");
...
Node::Expr *Parser::parse_call(PStruct *psr, Node::Expr *left, BindingPower bp) {
  ...
  while (psr->current(psr).kind != TokenKind::RIGHT_PAREN) {
    ...
    psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after an argument in a call expr!");
  }
...
Node::Expr *Parser::externalCall(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::CALL, "Expected a CALL keyword to start a call stmt");
  psr->expect(psr, TokenKind::LESS, "Expected a LESS to start call function name");
  
  std::string funcName = psr->expect(psr, TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a function name in a call stmt").value;

  psr->expect(psr, TokenKind::GREATER, "Expected a GREATER to end call function name");
  psr->expect(psr, TokenKind::LEFT_PAREN, "Expected a LEFT_PAREN to start call function arguments");
  
  std::vector<Node::Expr *> args;
  while (psr->current(psr).kind != TokenKind::RIGHT_PAREN) {
    args.push_back(parseExpr(psr, BindingPower::defaultValue));
    if (psr->current(psr).kind == TokenKind::RIGHT_PAREN) break;
    psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after an arguement in a call stmt");
  }

  psr->expect(psr, TokenKind::RIGHT_PAREN, "Expected a RIGHT_PAREN to end call function arguments");
...
Node::Expr *Parser::structExpr(PStruct *psr) {
  int line = psr->tks[psr->pos].line;
  int column = psr->tks[psr->pos].column;
  psr->expect(psr, TokenKind::LEFT_BRACE, "Expected a L_Brace to start a struct expr!");

  std::unordered_map<std::string, Node::Expr *> elements;
  while (psr->current(psr).kind != TokenKind::RIGHT_BRACE) {
    std::string key = psr->expect(psr, TokenKind::IDENTIFIER, "Expected an IDENTIFIER as a key in a struct expr!").value;
    psr->expect(psr, TokenKind::COLON, "Expected a COLON after a key in a struct expr!");
    Node::Expr *value = parseExpr(psr, defaultValue);
    elements[key] = value;

    if (psr->current(psr).kind == TokenKind::RIGHT_BRACE) break;
    psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after a key-value pair in a struct expr!");
  }

  psr->expect(psr, TokenKind::RIGHT_BRACE, "Expected a R_Brace to end a struct expr!");
...
Node::Expr *Parser::openExpr(PStruct *psr) {
  ...
  psr->expect(psr, TokenKind::OPEN, "Expected an OPEN keyword to start an open expr!");
  psr->expect(psr, TokenKind::LEFT_PAREN, "Expected a L_Paren to start an open expr!");
  ...
  while (psr->current(psr).kind != TokenKind::RIGHT_PAREN) {
    if (canRead == nullptr) {
      ...
      psr->expect(psr, TokenKind::COMMA, "Expected a COMMA after an argument in an open expr!");
    }
  }

  // this time there MUST be a closing parenthesis
  psr->expect(psr, TokenKind::RIGHT_PAREN, "Expected a R_Paren to end an open expr!");
