...
void codegen::gen(Node::Stmt *stmt, bool isSaved, std::string output_filename,
  ...
  if (nativeFunctionsUsed[NativeASMFunc::itoa] == true) {
    file << ".type native_itoa, @function\n"
          ...
          "\n    jnz .itoa_check_sign    # If not zero, process the digits"
          ...
          "\n    testq %rdi, %rdi        # Check if %rdi < 0"
          "\n    jns .itoa_loop          # If positive, skip sign handling"
          "\n    # Convert negative to positive (negate)"
          "\n    negq %rax               # %rax = -%rax"
          "\n    movb $'-', %r8b         # Store '-' in temp register"
          "\n.itoa_loop:"
          "\n    # Process each digit"
          ...
          "\n    jnz .itoa_div           # If not zero, divide again"
          "\n    # Restore '-' sign if negative"
          "\n    testb %r8b, %r8b        # Check if negative"
          "\n    jz .itoa_return         # If not negative, return"
          "\n    decq %rcx               # Move buffer pointer"
          "\n    movb %r8b, (%rcx)       # Store '-' sign"
          "\n.itoa_return:"
          "\n    leaq (%rcx), %rax       # Return pointer to the string in %rax"
          "\n    addq $32, %rsp          # Restore the stack"
          "\n    ret"
