...
void codegen::call(Node::Expr *expr) {
	...
	push(Instr{.var = Comment{.comment = "Call function '" + callee + "'"},
						 .type = InstrType::Comment},
			 Section::Main);
	push(Instr{.var = CallInstr{.name = "usr_" + callee}, .type = InstrType::Call},
			 Section::Main);
	push(Instr{.var = PushInstr{.what = "%rax"}, .type = InstrType::Push},
			 Section::Main);
	stackSize++;
...
void codegen::primary(Node::Expr *expr) {
	...
	case ND_NUMBER: {
		...
		push(Instr{.var = PushInstr{.what = '$' + res, .whatSize = DataSize::Qword}, .type = InstrType::Push},
				 Section::Main);
		stackSize++;
		break;
	}
	case ND_IDENT: {
		...
		push(Instr{.var =
				 ...
				 Section::Main);
		 if (offset != 0) {
			push(Instr{.var = PushInstr{.what = std::to_string(offset) + "(%rsp)", .whatSize = DataSize::Qword},
									.type = InstrType::Push}, 
									Section::Main);
			stackSize++;
			break;
	...
	case ND_STRING: {
		...
		push(Instr{.var = PushInstr{.what = '$' + label, .whatSize = DataSize::Qword}, .type = InstrType::Push},
				 Section::Main);
		stackSize++;
		...
		push(Instr{.var = DBInstr{.what = string->value + ", 00"},
							 .type = InstrType::Asciz}, // Asciz should automatically add a 00 but we dont talk about that!
				 Section::Data);
...
void codegen::assign(Node::Expr *expr) {
	...
	if (offset == 0) {
		push(Instr{.var = PopInstr{.where = "(%rsp)", .whereSize = DataSize::Qword}, .type = InstrType::Pop},
				 Section::Main);
		stackSize--;
		return;
	}
