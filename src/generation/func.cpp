...
void Gen::functionDeclaration(std::ofstream &file,
                              std::unique_ptr<AstNode> node) {
  std::unique_ptr<AstNode::FunctionDeclaration> fun =
      std::unique_ptr<AstNode::FunctionDeclaration>(
          (AstNode::FunctionDeclaration *)node->data);

  const char *type = findType(fun->type.get());
  std::cout << "Type: " << type;
  ...
  if (fun->parameters.size() > 0) {
    for (Lexer::Token parameter : fun->parameters) {
      ...
      case AstNodeType::TYPE: {
        AstNode::Type *typeNode = (AstNode::Type *)fun->paramType.front()->data;
