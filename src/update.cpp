...
std::string getTag() {
  ...
  if (osName == "linux" || osName == "darwin") {
    try {
      std::string result;
      FILE *pipe = popen("curl -s "
                         "'https://api.github.com/repos/TheDevConnor/"
                         "Zura-Transpiled/releases'",
                         "r");
      if (!pipe) {
    ...
    try {
      std::string result;
      std::string command = "powershell -Command \"(Invoke-RestMethod -Uri "
                            "'https://api.github.com/repos/TheDevConnor/"
                            "Zura-Transpiled/releases').tag_name\"";
...
void installer() {
  ...
  if (osName == "windows") {
    std::string url =
        ...
        tag + "/zura.exe";
    std::string command =
        "powershell -Command \"Invoke-WebRequest " + url + "\"";
    try {
