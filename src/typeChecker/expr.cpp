...
void TypeChecker::visitFloat(Node::Expr *expr) {
  ...
  if ((float)value > std::numeric_limits<float>::max() ||
      (float)value < std::numeric_limits<float>::min()) {
    // Check if it's out of range for a DOUBLE
    if ((double)value > std::numeric_limits<double>::max() ||
...
void TypeChecker::visitIdent(Node::Expr *expr) {
  ...
  //     ident->name, ident->line, ident->pos}); return;
  //   }
  ...
  //   ident->name, ident->line, ident->pos});
  // }
}
...
void TypeChecker::visitCall(Node::Expr *expr) {
  ...
  if (context->functionTable.find({fnName.first, fnName.second}) !=
      context->functionTable.end()) {
  ...
  // if (context->functionTable.find({fnName.first, fnName.second}) !=
  // context->functionTable.end()) {
  //   fnParams = context->functionTable.at({fnName.first,
  //   fnName.second}).params;
  // }
  ...
  for (auto it : context->functionTable)
    return_type = share(it.first.type);
  expr->asmType = createDuplicate(return_type.get());

  // add an ident for the lsp
  if (isLspMode) {
    lsp_idents.push_back(
        LSPIdentifier{fnName.second, LSPIdentifierType::Function, fnName.first,
...
void TypeChecker::visitArrayAutoFill(Node::Expr *expr) {
  ...
  array->fillCount = (size_t)arrayType->constSize;
...
void TypeChecker::visitStructExpr(Node::Expr *expr) {
  ...
  for (auto it : context->structTable[struct_name])
    struct_size++;
