...
void TypeChecker::visitTemplateStmt(Node::Stmt *stmt) {
  ...
  // }
  std::cout << "TemplateStmt not implemented yet. Attempted on function named '"
            << templateStmt->typenames[0] << "'" << std::endl;
  exit(-1);
}
...
void TypeChecker::visitImport(Node::Stmt *stmt) {
  ...
  // map->global_symbol_table.find(import_stmt->name); if (res !=
  // map->global_symbol_table.end()) {
  //   std::string msg = "'" + import_stmt->name + "' has already been
  //   imported."; handleError(import_stmt->line, import_stmt->pos, msg, "",
  //   "Type Error");
  // }
  std::unordered_map<std::string, Node::Type *>::iterator res =
